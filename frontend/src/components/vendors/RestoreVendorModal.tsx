...
interface RestoreVendorModalProps {
    open: boolean;
...
export function RestoreVendorModal({ open, onOpenChange, vendor, onConfirm }: RestoreVendorModalProps) {
    ...
    return (
        <DialogRoot open={open} onOpenChange={(e) => onOpenChange(e.open)}>
            <DialogContent>
                ...
                <DialogBody>
                    <p className="text-sm text-[var(--fg-muted)]">{t("restoreDialog.description", { name: vendor?.name || "" })}</p>
                </DialogBody>
                <DialogFooter>
                    ...
                    <button onClick={() => void handleConfirm()} disabled={isSubmitting} className="px-4 py-2 rounded-lg bg-green-600 text-white text-sm font-semibold hover:bg-green-700 transition-colors disabled:opacity-50 flex items-center gap-2">
